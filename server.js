...
app.post('/wallet/withdraw', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!transferResponse.ok || !transferData?.status) {
      ...
      if (transferErrorCode === 'paystack_business_tier_restricted') {
        const nowIso = new Date().toISOString();
        ...
        await adminDb.collection('transactions').add({
          ...
          commission: 0,
          netAmount: amount,
          paymentMethod: 'Manual Transfer Queue',
          status: 'MANUAL_REVIEW',
        ...
        await Promise.all(
          ADMIN_EMAILS.map((adminEmail) => writeNotification(
            adminEmail,
            `Manual withdrawal required: ${actorEmail} requested GHS ${amount.toFixed(2)} to ${provider} (${normalizedPhone}).`
          ))
        );

        await notifyUser(
          actorEmail,
          `Your withdrawal of GHS ${amount.toFixed(2)} has been queued for manual processing. You will be notified once completed.`,
          'Withdrawal Queued',
          { screen: 'wallet' }
        );
