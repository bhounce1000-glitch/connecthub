...
function validateChallenge(challenge, face) {
  const { yaw, pitch } = extractFaceAngles(face);
  if (challenge === 'LOOK_LEFT') return yaw < -12;
  if (challenge === 'LOOK_RIGHT') return yaw > 12;
  if (challenge === 'LOOK_UP') return pitch < -8;
  if (challenge === 'LOOK_DOWN') return pitch > 8;
  return Math.abs(yaw) <= 8 && Math.abs(pitch) <= 8;
}
...
export default function KycStepFace() {
  ...
  const captureAndDetect = async () => {
    ...
    return { photo, faces: result?.faces || [] };
  };
    ...
    try {
      for (let i = 0; i < CHALLENGE_SEQUENCE.length; i += 1) {
        ...
        while (!passed && attempts < 5) {
          ...
          const { faces } = await captureAndDetect();

          if (faces.length > 1) {
            setNotice({ type: 'error', message: 'Multiple faces detected. Move to a private, well-lit area and retry.' });
          ...
          if (validateChallenge(challenge.key, faces[0])) {
        ...
        if (!passed) {
          throw new Error(`Could not confirm: ${challenge.instruction}. Please retry.`);
        }
      }
      ...
      while (held < 8) {
        ...
        const { photo, faces } = await captureAndDetect();
        ...
        if (!validateChallenge('LOOK_CENTER', faces[0])) {
          held = 0;
          setHoldFrames(0);
          continue;
        }
      ...
      setNotice({ type: 'success', message: 'Face liveness verified. Continue to review and submit.' });
      router.push('/kyc/step4');
    } catch (error) {
      if (String(error?.message || '').toLowerCase() !== 'multiple faces detected') {
        ...
        <ScrollView contentContainerStyle={{ alignItems: 'center', paddingHorizontal: AppSpace.lg, paddingTop: AppSpace.xl, paddingBottom: 40 }}>
          <View style={{ width: '100%', maxWidth: maxW }}>
            ...
            <Text style={{ color: AppColors.ink500, fontSize: AppType.body, marginBottom: AppSpace.xl }}>
              Step 4 of 5 - We verify you are physically present before final KYC submission.
            </Text>
            ...
            <View style={{ backgroundColor: '#0f172a', borderRadius: AppRadius.lg, padding: AppSpace.lg, borderWidth: 1, borderColor: '#1e293b' }}>
              ...
              <Text style={{ color: '#94a3b8', lineHeight: 21, marginBottom: 14 }}>
                You will be asked to look left, right, up, down, then center. Keep your face in-frame in good lighting.
              </Text>
